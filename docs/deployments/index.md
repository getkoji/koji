...
- **Every credential has a scope.** When you add one, *Available to* decides whether it belongs to the current project only (the default) or to **all projects in this workspace**. A shared credential is usable from every project, including ones created later — so a new project isn't a blank slate. A credential added to a single project **overrides** the shared one for that project and nothing else, which is how you point one project at a different model or OCR vendor without disturbing the rest. Only a member who can reach every project may create, change, or delete a shared credential.
