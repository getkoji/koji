...
Per-document failures are also **persisted on the run record** (not just returned in the response): the run row carries `docsFailed` and a `failuresJson` array with the same `{ entryId, filename, error }` shape. If every document fails, the run is marked `failed` with an `errorMessage`, and the endpoint returns `502` with the `parseFailures` detail. `GET /api/schemas/{slug}/performance` returns `status`, `errorMessage`, `docsFailed`, and `failuresJson` for each run, so failed runs stay diagnosable after the fact — the dashboard Performance page renders them as failed with the per-document errors instead of a bare `0/N`.
