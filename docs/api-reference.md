...
Each entry in `fields[]` carries `name`, `accuracy` (%), `prevAccuracy`, `status`, and `failingDocs`. `status` is `pass`, `failing`, `regressed`, or `not_in_schema` — the last meaning ground truth carries the field but the schema being validated declares no such field, so nothing extracted it and its `accuracy` says nothing about extraction quality. Without a `yaml` body the run scores the **released** version, so a comparison against a validated local file is only like-for-like if the two declare the same fields. **Array fields** are scored by F1 and additionally carry `precision` and `recall` (percentages), so a low score can be read as missed elements (low recall) vs spurious/wrong elements (low precision). Element matching uses the array's `element_key` hint when declared, and sub-fields marked `informational` are excluded from scoring (see the schema guide).
