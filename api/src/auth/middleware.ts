...
export function authMiddleware(adapter: AuthAdapter, opts: AuthMiddlewareOptions = {}) {
  ...
  return async (c: Context<Env>, next: Next) => {
    ...
    // Document preview — validated via time-limited HMAC token.
    // The token is generated by the trace-detail API and appended as ?token=...
    // PdfViewer fetches via JS so it can't rely on session cookies.
    if (path.match(/^\/api\/jobs\/[^/]+\/documents\/[^/]+\/(preview|embed-data)$/)) {
      const masterKey = c.get("masterKey") as string | null;
      if (!masterKey) {
        // No master key (local dev) — allow unauthenticated preview
        await next();
        return;
      }
      const previewToken = c.req.query("token");
      if (previewToken && verifyPreviewToken(previewToken, path, masterKey)) {
        await next();
        return;
      }
