...
function apiJobsToRows(apiJobs: JobRow[]) {
  return apiJobs.map((j) => ({
    ...
    cost: j.totalCostUsd ? `$${j.totalCostUsd}` : "$0.00",
    started: j.startedAt ? new Date(j.startedAt).toLocaleString() : "—",
  }));
}

export default function JobsPage() {
  const { data: rows, live } = useApi(
    () => jobsApi.list().then(apiJobsToRows),
    MOCK_JOBS,
  );
    ...
    <ListLayout
      header={
        <>
          ...
          <PageHeader
            title="Jobs"
            badge={live ? <span className="font-mono text-[9px] text-green uppercase tracking-[0.1em]">live</span> : undefined}
            actions={
      ...
      filterBar={
        <div className="flex items-center gap-2">
          ...
          <span className="font-mono text-[10px] text-ink-4">{rows.length} total</span>
        </div>
      }
    >
