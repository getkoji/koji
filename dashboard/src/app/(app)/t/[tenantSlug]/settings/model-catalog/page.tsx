    ...
    <div className="space-y-6">
      <section>
        ...
        <p className="text-[12.5px] text-ink-3 mb-4">
          Models available for use in extraction pipelines. Add models manually or fetch them from a configured provider.
        </p>
        ...
        {Object.keys(grouped).length > 0 ? (
                  ...
                  {providerModels.map((m) => (
                    <SettingsRow key={m.id}>
                      <div className="flex items-center gap-4">
                        ...
                        {m.contextWindow && (
                          <Meta>{(m.contextWindow / 1000).toFixed(0)}k ctx</Meta>
                        )}
          ...
          <div className="border border-border rounded-sm py-6 text-center text-[12.5px] text-ink-3">
            No models in the catalog yet. Add models manually or fetch them from a provider.
          </div>
        )}
      </section>
...
function AddModelDialog({ onClose, onAdded }: { onClose: () => void; onAdded: () => void }) {
  ...
  // For fetch mode — need to pick an endpoint
  const [endpointId, setEndpointId] = useState("");
  const [fetching, setFetching] = useState(false);
  const [fetchResult, setFetchResult] = useState<string | null>(null);

  const { data: endpoints } = useApi(
    useCallback(() => api.get<{ data: Array<{ id: string; displayName: string; provider: string }> }>("/api/model-providers").then((r) => r.data), []),
  );
  ...
  async function handleFetch() {
    ...
    setFetching(true);
    try {
      const result = await api.post<{ fetched: number; provider: string }>("/api/model-catalog/fetch", { endpoint_id: endpointId });
      setFetchResult(`Fetched ${result.fetched} models from ${result.provider}`);
        ...
        {mode === "manual" ? (
          <form onSubmit={handleManualSubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1.5">
                ...
                <input required value={modelId} onChange={(e) => setModelId(e.target.value)} placeholder="gpt-4o"
            ...
            <div className="space-y-1.5">
              ...
              <input value={displayName} onChange={(e) => setDisplayName(e.target.value)} placeholder="GPT-4o (defaults to model ID)"
          ...
          <div className="space-y-4">
            <p className="text-[12.5px] text-ink-3">
              Select a configured provider to fetch its available models automatically.
            </p>

            <div className="space-y-1.5">
              <label className="text-[12.5px] font-medium text-ink">Provider endpoint</label>
              <select value={endpointId} onChange={(e) => setEndpointId(e.target.value)}
                className="w-full h-[30px] rounded-sm border border-input bg-white px-2 text-[13px] outline-none focus:border-ring focus:ring-[2px] focus:ring-ring/30">
                <option value="">Select a provider...</option>
                {(endpoints ?? []).map((ep) => (
                  <option key={ep.id} value={ep.id}>{ep.displayName} ({ep.provider})</option>
                ))}
              </select>
            </div>
            ...
            {(endpoints ?? []).length === 0 && (
              <p className="text-[12px] text-ink-4">
                No providers configured yet. Add a provider in Project Settings first.
              </p>
            )}

            {error && <div className="text-[12px] text-vermillion-2 bg-vermillion-3/50 px-3 py-1.5 rounded-sm">{error}</div>}
            {fetchResult && <div className="text-[12px] text-green bg-green/10 px-3 py-1.5 rounded-sm">{fetchResult}</div>}
            ...
            <div className="flex items-center justify-end gap-2 pt-1">
              ...
              <button onClick={handleFetch} disabled={!endpointId || fetching}
