...
def _strip_label(text: str, label: str | None, field_name: str) -> str:
    ...
    1. If user provided an explicit label, strip lines matching it
    2. Auto-detect: if first line looks like the field name, strip it

    Handles variations: "INSURED", "INSURED:", "Named Insured", etc.
    """
    if not text:
        return text
    ...
    if label:
        ...
        for line in lines:
            ...
            # Skip lines that match the label (exact, contains, or contained by)
            if line_lower == label_lower or line_lower in label_lower or label_lower in line_lower:
                continue
            cleaned.append(line)
        if cleaned:
            return "\n".join(cleaned)
        return text  # Don't return empty if everything was stripped

    # Auto-detect: if first line looks like a form label
    # (short, ALL CAPS or matches field name pattern, followed by actual content)
    if len(lines) > 1:
        ...
        is_label = (
            len(first.split()) <= 3  # Short (1-3 words)
            and (
                first == first.upper()  # ALL CAPS
                or first_lower in field_words  # Matches field name
                or field_words in first_lower
            )
        )
        if is_label:
            return "\n".join(lines[1:])
...
def _extract_words_in_box(page, x0: float, y0: float, x1: float, y1: float) -> str:
    ...
    """
    # Try pymupdf (fitz) first — it handles overlapping text objects better
    # than pdfplumber for form PDFs generated by insurance systems
    try:
        import fitz as pymupdf

        # We need the original file path, not a page object from pdfplumber.
        # Access the PDF filename from the page's parent pdf object.
        pdf_path = page.pdf.stream.name if hasattr(page.pdf.stream, "name") else None
        if pdf_path:
            ...
            if text and text.strip():
                ...
                result = "\n".join(lines)
                print(f"[extract_words_in_box] pymupdf result: {result[:100]}")
                return result
    except ImportError:
        print("[extract_words_in_box] pymupdf not available, falling back")
    except Exception as e:
        print(f"[extract_words_in_box] pymupdf failed: {e}")

    # Fallback to pdfplumber word-level extraction
    words = page.extract_words(keep_blank_chars=False)
    print(f"[extract_words_in_box] Fallback pdfplumber: {len(words)} words")
    ...
    print(f"[extract_words_in_box] Words inside box: {len(inside)}")
    if inside:
        print(f"[extract_words_in_box] First few: {[w['text'] for w in inside[:5]]}")
