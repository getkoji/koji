...
async def extract(req: ExtractionRequest):
    ...
    start = time.time()
    print(f"[koji-extract] Request: model={model!r} (req.model={req.model!r}, default={DEFAULT_MODEL!r}), strategy={strategy}, markdown={len(req.markdown)} chars, fields={list(req.schema_def.get('fields', {}).keys())}")
